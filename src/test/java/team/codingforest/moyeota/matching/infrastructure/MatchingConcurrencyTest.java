...
@Import({PartyApplicationService.class, PartyJpa.class, ActiveMatchParticipationStore.class,
        MatchingTransactions.class, ZooKeeperMatchingAdmission.class, MatchingZooKeeperConfig.class, UserJpa.class, JpaAuditingConfig.class})
...
class MatchingConcurrencyTest {
    ...
    @AfterAll static void closeCluster() throws Exception { cluster.close(); }
    ...
    void 잠금이_겹쳐도_DB_PK가_두계정이_아닌_동일사용자의_중복참여를_최종_차단한다() throws Exception {
        Long member = user(), a = room(), b = room();
        var barrier = new CyclicBarrier(2);
        // ZooKeeper 세션 만료/소유권 상실로 두 작업이 진입한 상황에 해당하는 DB 방어 시험.
    ...
    @Test
    void 저장후_세션_만료를_감지하면_커밋을_거절하고_참여를_롤백한다() throws Exception {
        var framework = org.apache.curator.framework.CuratorFrameworkFactory.builder()
                .connectString(cluster.getConnectString()).sessionTimeoutMs(4000).connectionTimeoutMs(2000)
                .retryPolicy(new org.apache.curator.retry.RetryOneTime(100)).build();
        try (var client = new ZooKeeperMatchingClient(framework)) {
            assertThat(framework.blockUntilConnected(10, TimeUnit.SECONDS)).isTrue();
            var provider = new org.springframework.beans.factory.support.StaticListableBeanFactory(Map.of("client", client))
                    .getBeanProvider(ZooKeeperMatchingClient.class);
            var isolated = new ZooKeeperMatchingAdmission(new MatchingTransactions(manager, dataSource), provider);
            var lost = new CountDownLatch(1);
            framework.getConnectionStateListenable().addListener((c, state) -> {
                if (state == org.apache.curator.framework.state.ConnectionState.LOST) lost.countDown();
            });
            Long member = user();
            long before = jdbc.queryForObject("select count(*) from match_room", Long.class);
            assertThatThrownBy(() -> isolated.execute(member, () -> {
                parties.save(Party.open(member, new Location(37.4979, 127.0276), new Location(37.3948, 127.1112),
                        "강남역", "판교역", new Capacity(2), Instant.now(), new Radius(100), new Radius(100), 12000, 25, "route"));
                try {
                    framework.getZookeeperClient().getZooKeeper().getTestable().injectSessionExpiration();
                    assertThat(lost.await(3, TimeUnit.SECONDS)).isTrue();
                } catch (Exception e) { throw new IllegalStateException(e); }
                return true;
            })).isInstanceOf(BusinessException.class).extracting("errorCode").isEqualTo(MATCHING_LOCK_UNAVAILABLE);
            assertThat(count(member)).isZero();
            assertThat(jdbc.queryForObject("select count(*) from match_room", Long.class)).isEqualTo(before);
        }
    }
